...
class PurchasebillController extends Controller
{
    ...
    {
        $check = Table_visible::where('user_id', '=', $this->auth_id)->where('name','=','Purchaselist')->first();
        if(!empty($check))
        {
        ...
        return view('dashboard.purchasebilllist',compact('array_check','add_row','update_row','delete_row','excel','pdf','print','col_visible','show_row'));
    }
...
public function getlist(Request $request){
    ...
    $totalData = Tbl_invoice::Where('user_id', '=', $this->auth_id)->Where(function ($query) use ($search) {
    ...
    if(empty($request->input('search.value'))){
        ...
        ->orderBy($order,$dir)
        ->get();
        $totalFiltered = Tbl_invoice::Where('user_id', '=', $this->auth_id)->Where(function ($query) use ($search) {
                              $query->orwhere('bill_type', '=', '3')
                                    ->orwhere('bill_type', '=', '4');
                            })->count();
    }else{
    ...
    if($posts){
        foreach($posts as $r){
            ...
            if($r->bill_type == '4')
            {
                ...
                $convert = '<p style="color:green;">'.trans('app.Converted to bill').'</p>';
            }
            else
            {
                $convert = '<span class="btn-sm btn-info btn-xs mr-1" onclick="convert_to_bill('.$r->id.')">'.trans('app.Convert to bill').'</span>';
            }
            ...
            {
                $update_delete = '<a href="'.route('purchasebills.edit',[app()->getLocale(),$r->id]).'" class="btn-sm btn-info btn-xs"><i class="fas fa-edit"></i></a>';
            }
            else
            {
                $update_delete = '<a href="'.route('purchasebills.edit',[app()->getLocale(),$r->id]).'" class="btn-sm btn-info btn-xs"><i class="fas fa-edit"></i></a><span class="btn-sm btn-danger btn-xs" onclick="dlt_data('.$r->id.')"><i class="fas fa-trash-alt"></i></span>';
            }
            ...
            $nestedData[] = '<div class="row"><span class="btn-sm btn-info btn-xs mr-1" onclick="check_info('.$r->id.')"><i class="fas fa-info"></i></span><a href="'.route('purchasebill',[app()->getLocale(),$r->id]).'" class="btn-sm btn-primary btn-xs" onclick="check_info('.$r->id.')"><i class="fas fa-file-invoice"></i></a></div>';
...
public function purchasebill($prefix,$id){
    $user_id = $this->auth_id;
    $user  = User::where('id', '=', $user_id)->get();
    $invoice = Tbl_invoice::where('id', '=', $id)->first();
    $invoice_product = Tbl_invoice_product::where('invoice_id', '=', $id)->get();
    return view('dashboard.purchasebill',compact('invoice','invoice_product','user'));
}
...
public function data($prefix,Request $request){
    ...
    <h3 class="card-title">'.trans('app.Purchase Info').'</h3>
    </div>
    ...
    <th>'.trans('app.CGST').'</th>
    <th>'.trans('app.SGST').'</th>
    <th>'.trans('app.IGST').'</th>
    <th>'.trans('app.Total Amount').'</th>
    </tr>
    '.$tabl_info.'
    <tr>
    <td colspan="9" style="text-align:right;">'.trans('app.Net Amount').'</td>
...
public function create()
{
    ...
    {
       $id = $this->auth_id;
       $client_data  = Tbl_client::where('user_id', '=', $id)->where('type', '!=', '0')->get();
       $product_data  = Tbl_product::where('user_id', '=', $id)->get();
       return view('dashboard.addpurchasebill',compact('client_data','product_data'));
    }else{
    return redirect()->route('home',app()->getLocale())->with('message', 'Not Found.');
    }
}
...
public function store(Request $request)
{
    ...
    $invoicedata['total_sgst'] = $data['totalsgst'];
    $invoicedata['total_cgst'] = $data['totalcgst'];
    $invoicedata['total_igst'] = $data['totaligst'];
    ...
    $invoicedata['bill_type'] = '3';

    $invoice = Tbl_invoice::create($invoicedata);
    $invoice_id = $invoice->id;

    $no_type = Auth::user()->purchse_no;
    $no_date_type = Auth::user()->purchse_no_date;
    ...
    for ($i=1; $i <= $total_product_add ; $i++) {
        if($data['remove_product'.$i] == '1')
        {
            ...
            $productdata['cgst'] = $data['cgst'.$i];
            $productdata['sgst'] = $data['sgst'.$i];
            $productdata['igst'] = $data['igst'.$i];
    ...
    return redirect()->route('purchasebills.index',app()->getLocale())
...
public function edit($prefix,$id)
{
    ...
    {
        $user_id = $this->auth_id;
        $client_data  = Tbl_client::where('user_id', '=', $user_id)->where('type', '!=', '0')->get();
        ...
        return view('dashboard.addpurchasebill',compact('Tbl_invoice','Tbl_invoice_product','client_data','product_data'));
    }
    else
    {
        return redirect()->route('home',app()->getLocale())->with('message', 'Not Found.');
...
public function update(Request $request,Tbl_product $Tbl_product)
{
   ...
   $total_product_add = $data['total_product_add'];
    ...
    return redirect()->route('purchasebills.index',app()->getLocale())
     ->with('success','Purchase Bill Update successfully.');
}



public function converttobill($prefix,Request $request)
{
    $update['bill_type'] = '4';
    Tbl_invoice::find($request->input('id'))->update($update);
}
...
public function multydestroy($prefix,Request $request)
{
    ...
    return redirect()->route('purchasebills.index',app()->getLocale())
    ->with('success','Purchase Bill delete successfully.');
}
}
