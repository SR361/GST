...
class QuotebillController extends Controller
{
    private $add_row;
    private $update_row;
    private $delete_row;
    ...
    {
        $check = Table_visible::where('user_id', '=', $this->auth_id)->where('name','=','Quotelist')->first();
        if(!empty($check))
        {
        ...
        return view('dashboard.quotebilllist',compact('array_check','add_row','update_row','delete_row','excel','pdf','print','col_visible','show_row'));
    }
...
public function getlist(Request $request){
    ...
    $totalData = Tbl_invoice::where('user_id', '=', $this->auth_id)->Where('bill_type', '=', '2')->count();
    ...
    if(empty($request->input('search.value'))){
        ...
        ->orderBy($order,$dir)
        ->get();
        $totalFiltered = Tbl_invoice::where('user_id', '=', $this->auth_id)->Where('bill_type', '=', '2')->count();
    }else{
    ...
    if($posts){
        foreach($posts as $r){
            ...
            {
                $update_delete = '<a href="'.route('quotebills.edit',[app()->getLocale(),$r->id]).'" class="btn-sm btn-info btn-xs"><i class="fas fa-edit"></i></a>';
            }
            else
            {
                $update_delete = '<a href="'.route('quotebills.edit',[app()->getLocale(),$r->id]).'" class="btn-sm btn-info btn-xs"><i class="fas fa-edit"></i></a><span class="btn-sm btn-danger btn-xs" onclick="dlt_data('.$r->id.')"><i class="fas fa-trash-alt"></i></span>';
            }
            ...
            $nestedData[] = '<div class="row"><span class="btn-sm btn-info btn-xs mr-1" onclick="check_info('.$r->id.')"><i class="fas fa-info"></i></span><a href="'.route('quotebill',[app()->getLocale(),$r->id]).'" class="btn-sm btn-primary btn-xs" onclick="check_info('.$r->id.')"><i class="fas fa-file-invoice"></i></a></div>';
            ...
            $nestedData[] = '<a href="'.route('quotebills.edit',[app()->getLocale(),$r->id]).'" class="btn-sm btn-info btn-xs"><i class="fas fa-edit"></i></a><span class="btn-sm btn-danger btn-xs" onclick="dlt_data('.$r->id.')"><i class="fas fa-trash-alt"></i></span>';
            $data[] = $nestedData;
        }
    }
...
public function data($prefix,Request $request){
    ...
    <h3 class="card-title">'.trans('app.Quote Info').'</h3>
    </div>
...
public function create()
{
    ...
    if($add_row == '1')
    {
        ...
        return view('dashboard.addquotebill',compact('client_data','product_data'));
    }else{
        return redirect()->route('home',app()->getLocale())->with('message', 'Not Found.');
    }
}
...
public function store(Request $request)
{
    ...
    $invoicedata['bill_type'] = '2';

    $invoice = Tbl_invoice::create($invoicedata);
    $invoice_id = $invoice->id;

    $no_type = Auth::user()->quote_no;
    $no_date_type = Auth::user()->quote_no_date;
    ...
    return redirect()->route('quotebills.index',app()->getLocale())
     ->with('success','Quote Bill created successfully.');
}
...
public function edit($prefix,$id)
{
    ...
    if($update_row == '1')
    {
        ...
        return view('dashboard.addquotebill',compact('Tbl_invoice','Tbl_invoice_product','client_data','product_data'));
    }
    else
    {
        return redirect()->route('home',app()->getLocale())->with('message', 'Not Found.');
...
public function update(Request $request,Tbl_product $Tbl_product)
{
   ...
   $total_product_add = $data['total_product_add'];
    ...
    return redirect()->route('quotebills.index',app()->getLocale())
     ->with('success','Quote Bill Update successfully.');
}
...
public function multydestroy($prefix,Request $request)
{
    ...
    return redirect()->route('quotebills.index',app()->getLocale())
    ->with('success','Quote Bill delete successfully.');
}
}
