...
class InvoiceController extends Controller
{
    ...
    {
        $check = Table_visible::where('user_id', '=', $this->auth_id)->where('name','=','Invoicelist')->first();
        if(!empty($check))
        {
        ...
        return view('dashboard.invoicelist',compact('array_check','add_row','update_row','delete_row','excel','pdf','print','col_visible','show_row'));
    }
...
public function getlist(Request $request){
    ...
    $totalData = Tbl_invoice::where('user_id', '=', $this->auth_id)->Where('bill_type', '=', '0')->count();
    ...
    if(empty($request->input('search.value'))){
        ...
        ->orderBy($order,$dir)
        ->get();
        $totalFiltered = Tbl_invoice::where('user_id', '=', $this->auth_id)->Where('bill_type', '=', '0')->count();
    }else{
    ...
    if($posts){
        foreach($posts as $r){
            ...
            {
                $update_delete = '<a href="'.route('invoices.edit',[app()->getLocale(),$r->id]).'" class="btn-sm btn-info btn-xs"><i class="fas fa-edit"></i></a>';
            }
            else
            {
                $update_delete = '<a href="'.route('invoices.edit',[app()->getLocale(),$r->id]).'" class="btn-sm btn-info btn-xs"><i class="fas fa-edit"></i></a><span class="btn-sm btn-danger btn-xs" onclick="dlt_data('.$r->id.')"><i class="fas fa-trash-alt"></i></span>';
            }
...
public function generatePDF($prefix,$id)
{
    ...
    $client_sel_data = Tbl_client::where(['id' => $invoice->client_id])->first();

    // $pdf = PDF::loadView('dashboard.new_invoice',compact('invoice','invoice_product','user','client_sel_data'));
    ...
    // $pdf = app('dompdf.wrapper')->loadView('dashboard.new_invoice', compact('invoice','invoice_product','user','client_sel_data'));
    // return $pdf->stream('invoice.pdf');
}

public function clientinfo($prefix,Request $request){
    $data = Tbl_client::where('id', '=',$request->input('id'))->first();
    return json_encode($data);
}

public function productinfo($prefix,Request $request){
    $data = Tbl_product::where('id', '=',$request->input('id'))->first();
    return json_encode($data);
}

public function invoice($prefix,$id){
    $user_id = $this->auth_id;
    $user  = User::where('id', '=', $user_id)->first();
    $invoice = Tbl_invoice::where('id', '=', $id)->first();
    $invoice_product = Tbl_invoice_product::where('invoice_id', '=', $id)->get();
    return view('dashboard.invoice',compact('invoice','invoice_product','user'));
}

public function addclient($prefix,Request $request)
{
   ...
   $client = Tbl_client::create($data);
   $newdata['id'] = $client->id;
   $newdata['company_name'] = $data['company_name'].'-'.$data['contact_person'];
   return json_encode($newdata);

}

public function addproduct($prefix,Request $request)
{
...
public function create()
{
    ...
    if($add_row == '1')
    {
        ...
        return view('dashboard.addinvoice',compact('client_data','product_data'));
    }else{
        return redirect()->route('home',app()->getLocale())->with('message', 'Not Found.');
    }
}
...
public function store(Request $request)
{
    ...
    $invoicedata['bill_type'] = '0';

    $invoice = Tbl_invoice::create($invoicedata);
    $invoice_id = $invoice->id;

    $no_type = Auth::user()->invoice_no;
    $no_date_type = Auth::user()->invoice_no_date;
    ...
    return redirect()->route('invoices.index',app()->getLocale())
...
public function edit($prefix,$id)
{
    ...
    if($update_row == '1')
    {
        ...
        return view('dashboard.addinvoice',compact('Tbl_invoice','Tbl_invoice_product','client_data','product_data'));
    }
    else
    {
        return redirect()->route('home',app()->getLocale())->with('message', 'Not Found.');
