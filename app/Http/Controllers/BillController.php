...
class BillController extends Controller
{
    ...
    {
        $check = Table_visible::where('user_id', '=', $this->auth_id)->where('name','=','Billlist')->first();
        if(!empty($check))
        {
        ...
        return view('dashboard.billlist',compact('array_check','add_row','update_row','delete_row','excel','pdf','print','col_visible','show_row'));
    }
...
public function getlist(Request $request){
    ...
    $totalData = Tbl_invoice::where('user_id', '=', $this->auth_id)->Where('bill_type', '=', '4')->count();
    ...
    if(empty($request->input('search.value'))){
        ...
        ->orderBy($order,$dir)
        ->get();
        $totalFiltered = Tbl_invoice::where('user_id', '=', $this->auth_id)->Where('bill_type', '=', '4')->count();
    }else{
    ...
    if($posts){
        foreach($posts as $r){
            ...
            $nestedData[] = '<div class="row"><span class="btn-sm btn-info btn-xs mr-1" onclick="check_info('.$r->id.')"><i class="fas fa-info"></i></span><a href="'.route('bill',[app()->getLocale(),$r->id]).'" class="btn-sm btn-primary btn-xs" onclick="check_info('.$r->id.')"><i class="fas fa-file-invoice"></i></a></div>';
            ...
            $nestedData[] = '<span class="btn-sm btn-danger btn-xs" onclick="dlt_data('.$r->id.')"><i class="fas fa-trash-alt"></i></span>';
            $data[] = $nestedData;
        }
    }
...
public function data($prefix,Request $request){
    ...
    <h3 class="card-title">'.trans('app.Bill Info').'</h3>
    </div>
    ...
    '.$tabl_info.'
    <tr>
    <td colspan="10" style="text-align:right;">'.trans('app.Net Amount').'</td>
...
{
    $update['bill_type'] = '3';
    Tbl_invoice::find($id)->update($update);
}
...
public function multydestroy($prefix,Request $request)
{
    ...
    return redirect()->route('bills.index',app()->getLocale())
    ->with('success','Bills delete successfully.');
}
}
