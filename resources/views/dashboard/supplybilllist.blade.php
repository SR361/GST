...
<div class="content-wrapper">
	...
	<div class="content-header">
		<div class="container-fluid">
			<div class="row mb-2">
				<div class="col-sm-6">
					<h1 class="m-0 text-dark">{{ trans('app.Supply Bill List') }}</h1>
				</div>
				<!-- /.col -->
				<div class="col-sm-6">
					<ol class="breadcrumb float-sm-right">
						<li class="breadcrumb-item"><a href="{{ url(app()->getLocale().'/dashboard') }}">{{ trans('app.Home') }}</a></li>
						<li class="breadcrumb-item active">{{ trans('app.Supply Bill List') }}</li>
					</ol>
				</div>
				<!-- /.col -->
			</div>
  ...
  <div id="supplybillinfo" class="modal fade docs-example-modal-lg" role="dialog">
  <div class="modal-dialog modal-lg" style="max-width: 91%" >
    <div class="modal-content ">
      <div class="modal-header">
       <h4 class="modal-title">{{ trans('app.Supply Bill Info') }}</h4>
       <button type="button" class="close" data-dismiss="modal">&times;</button>

     </div>
     <div class="modal-body" id="supplybillinfodata">
    </div>
  ...
  <div class="row">
   <div class="col-12">
     ...
     <form action="{{ url(app()->getLocale().'/supplybillsmuldel') }}" method="post" onsubmit="return giveNotation()">
    {{ csrf_field() }}
      <div class="card">
        <div class="card-header" style="display: <?php if($delete_row == '0' && $add_row == '0'){ echo "none"; } ?>">
          ...
          <?php } ?>
          <?php if($add_row == '1'){ ?>
           <a href="{{ route('supplybills.create',app()->getLocale()) }}" class="btn btn-success btn-sm pull-right">{{ trans('app.Add Supply Bill') }}</a>
          <?php } ?>
        </div>

        <div class="card-body mailbox-messages " > <!-- style="overflow:scroll" -->
          <table id="supplybill_table" class="{{ Config::get('constants.TABLE_CLASS') }}" style="width:100%; text-align: center;"> <!-- class="table table-bordered table-hover table-striped mailbox-messages" -->
           <thead bgcolor="{{ Config::get('constants.TABLE_COLOR') }}">
            <tr style="color: {{ Config::get('constants.TABLE_FONT_COLOR') }}">
              ...
              <th>{{ trans('app.Supply Bill Number') }}</th>
...
<script>
	var table = $('#supplybill_table').DataTable( {
		"pagingType": "full_numbers",
		"processing": true,
		"serverSide": true,
		"order": [0],
		"ajax": {
			"url":"<?= route('supplybilldataProcessing',app()->getLocale()) ?>",
			"dataType":"json",
			"type":"POST",
  ...
  $('#supplybill_table').on( 'column-visibility.dt', function ( e, settings, column, state ) {
   var name = "Supplybilllist";
   ...
   else
   {
	...
	function dlt_data(id)
	{
		if (confirm("Are You Sure you want to delete?")) {
			$.ajax({  
				url: "{{ url(app()->getLocale().'/supplybillsdelete')}}/"+id,
				type: 'DELETE',
  ...
  {
    $.ajax({  
        url: "<?= route('supplybilldatainfo',app()->getLocale()) ?>",
        type: 'POST',
        data: {id:id,"_token":"<?= csrf_token() ?>"},
        success:function(data)  
        {
         $("#supplybillinfodata").html(data);
         $("#supplybillinfo").modal('show');
        }
      });
  }
</script>
