...
<div class="content-wrapper">
  ...
  <div class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1 class="m-0 text-dark">{{ trans('app.Purchase Bill List') }}</h1>
        </div>
        <!-- /.col -->
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="{{ url(app()->getLocale().'/dashboard') }}">{{ trans('app.Home') }}</a></li>
            <li class="breadcrumb-item active">{{ trans('app.Purchase Bill List') }}</li>
          </ol>
        </div>
        <!-- /.col -->
      </div>
  ...
  <div id="purchasebillinfo" class="modal fade docs-example-modal-lg" role="dialog">
  <div class="modal-dialog modal-lg" style="max-width: 91%" >
    <div class="modal-content ">
      <div class="modal-header">
       <h4 class="modal-title">{{ trans('app.Purchase Info') }}</h4>
       <button type="button" class="close" data-dismiss="modal">&times;</button>

     </div>
     <div class="modal-body" id="purchasebillinfodata">
    </div>
  ...
  <div class="row">
   <div class="col-12">
     ...
     <form action="{{ url(app()->getLocale().'/purchasebillsmuldel') }}" method="post" onsubmit="return giveNotation()">
    {{ csrf_field() }}
      <div class="card">
        <div class="card-header" style="display: <?php if($delete_row == '0' && $add_row == '0'){ echo "none"; } ?>">
          ...
          <?php } ?>
          <?php if($add_row == '1'){ ?>
           <a href="{{ route('purchasebills.create',app()->getLocale()) }}" class="btn btn-success btn-sm pull-right">{{ trans('app.Add Purchase Bill') }}</a>
          <?php } ?>
        </div>
        
        <div class="card-body mailbox-messages " > <!-- style="overflow:scroll" -->
          <table id="purchasebill_table" class="{{ Config::get('constants.TABLE_CLASS') }}"  style="width:100%; text-align: center;"> <!-- class="table table-bordered table-hover table-striped mailbox-messages" -->
...
<script>
  var table = $('#purchasebill_table').DataTable( {
    "pagingType": "full_numbers",
    "processing": true,
    "serverSide": true,
    "order": [0],
    "ajax": {
      "url":"<?= route('purchasebilldataProcessing',app()->getLocale()) ?>",
      "dataType":"json",
      "type":"POST",
    ...
    "columnDefs": [
        ...
        ,{
          "targets": [ 8 ],
          "visible": '<?php if(in_array('8', $array_check)){ echo false; }else{ echo true; } ?>',
          "searchable": '<?php if(in_array('8', $array_check)){ echo false; }else{ echo true; } ?>'
        },{
          "targets": [ 9 ],
          "visible": '<?php if(in_array('9', $array_check) || ($delete_row == '0' && $update_row == '0')){ echo false; }else{ echo true; } ?>',
          "searchable": '<?php if(in_array('9', $array_check) || ($delete_row == '0' && $update_row == '0')){ echo false; }else{ echo true; } ?>'
        }]
    } );

  $('#purchasebill_table').on( 'column-visibility.dt', function ( e, settings, column, state ) {
   var name = "Purchaselist";
  ...
  function dlt_data(id)
  {
    if (confirm("Are You Sure you want to delete?")) {
      $.ajax({  
        url: "{{ url(app()->getLocale().'/purchasebillsdelete')}}/"+id,
        type: 'DELETE',
  ...
  {
    $.ajax({  
        url: "<?= route('purchasebilldatainfo',app()->getLocale()) ?>",
        type: 'POST',
        data: {id:id,"_token":"<?= csrf_token() ?>"},
        success:function(data)  
        {
         $("#purchasebillinfodata").html(data);
         $("#purchasebillinfo").modal('show');
        }
      });
  }

  function convert_to_bill(id)
  {
    $.ajax({  
        url: "<?= route('convert_to_bill',app()->getLocale()) ?>",
        type: 'POST',
        data: {id:id,"_token":"<?= csrf_token() ?>"},
        success:function(data)  
        {
          swal("", "Purchase Order Covert to Bill successfully!", "success");
          table.ajax.reload();
        }
      });
  }
