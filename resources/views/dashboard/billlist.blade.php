...
<div class="content-wrapper">
  ...
  <div class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1 class="m-0 text-dark">{{ trans('app.Bill List') }}</h1>
        </div>
        <!-- /.col -->
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="{{ url(app()->getLocale().'/dashboard') }}">{{ trans('app.Home') }}</a></li>
            <li class="breadcrumb-item active">{{ trans('app.Bill List') }}</li>
          </ol>
        </div>
        <!-- /.col -->
      </div>
  ...
  <div id="billinfo" class="modal fade docs-example-modal-lg" role="dialog">
  <div class="modal-dialog modal-lg" style="max-width: 91%" >
    <div class="modal-content ">
      <div class="modal-header">
       <h4 class="modal-title">{{ trans('app.Bill Info') }}</h4>
       <button type="button" class="close" data-dismiss="modal">&times;</button>

     </div>
     <div class="modal-body" id="billinfodata">
    </div>
  ...
  <div class="row">
   <div class="col-12">
     ...
     <form action="{{ url(app()->getLocale().'/billsmuldel') }}" method="post" onsubmit="return giveNotation()">
    {{ csrf_field() }}
      <div class="card">
        ...
        <div class="card-body mailbox-messages " ><?php /*?>style="overflow:scroll"<?php */?>
          <table id="bill_table" class="{{ Config::get('constants.TABLE_CLASS') }}"  style="width:100%; text-align: center;">
...
<script>
  var table = $('#bill_table').DataTable( {
    ...
    "order": [0],
    "ajax": {
      "url":"<?= route('billdataProcessing',app()->getLocale()) ?>",
      "dataType":"json",
      "type":"POST",
    ...
    "columnDefs": [
        ...
        ,{
          "targets": [ 8 ],
          "visible": '<?php if(in_array('8', $array_check) || ($delete_row == '0')){ echo false; }else{ echo true; } ?>',
          "searchable": '<?php if(in_array('8', $array_check) || ($delete_row == '0')){ echo false; }else{ echo true; } ?>'
        }]
    } );

  $('#bill_table').on( 'column-visibility.dt', function ( e, settings, column, state ) {
   var name = "Billlist";
 ...
 function resize() {
    if ($(window).width() < 900) {
     $('#bill_table').addClass('table-responsive');
    }
    else {$('#bill_table').removeClass('table-responsive');}
}
...
$(document).ready( function() {
  ...
  function dlt_data(id)
  {
    if (confirm("Are You Sure you want to delete?")) {
      $.ajax({  
        url: "{{ url(app()->getLocale().'/billsdelete')}}/"+id,
        type: 'DELETE',
  ...
  {
    $.ajax({  
        url: "<?= route('billdatainfo',app()->getLocale()) ?>",
        type: 'POST',
        data: {id:id,"_token":"<?= csrf_token() ?>"},
        success:function(data)  
        {
         $("#billinfodata").html(data);
         $("#billinfo").modal('show');
        }
      });
  }
