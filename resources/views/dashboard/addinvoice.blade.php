...
<div class="content-header">
	<div class="container-fluid">
		<div class="row mb-2">
			<div class="col-sm-6">
				<h1 class="m-0 text-dark">{{ trans('app.Add Invoice') }}</h1>
			</div>
			<!-- /.col -->
			<div class="col-sm-6">
				<ol class="breadcrumb float-sm-right">
					<li class="breadcrumb-item"><a href="{{ url(app()->getLocale().'/dashboard') }}">{{ trans('app.Home') }}</a></li>
					<li class="breadcrumb-item"><a href="{{ url(app()->getLocale().'/invoices') }}">{{ trans('app.Invoice List') }}</a></li>
					<li class="breadcrumb-item active">{{ trans('app.Add Invoice') }}</li>
				</ol>
			</div>
			<!-- /.col -->
		</div>
...
<section class="content">
	<div class="col-md-12">
		<div class="col-md-12">
			...
			<form action="{{ route('invoices.update',[app()->getLocale(),$Tbl_invoice->id]) }}" method="POST"  id="form_data" enctype="multipart/form-data" autocomplete="off">
				<input type="hidden" value="{{ $Tbl_invoice->id }}" name="id"> 
				@method('PUT')      
				@else
				<form action="{{ route('invoices.store',app()->getLocale()) }}" method="POST" id="form_data"  enctype="multipart/form-data" autocomplete="off">     
					@endif
					@csrf
					<div class="card">
						<div class="card-body">
								...
								<div class="col-md-4 col-sm-4 col-xs-12">
									...
									<div class="row">
										<div class="col-md-12">
											<div class="form-group input-group">
												<div class="col-md-4 m-t-5">
													<label>{{ trans('app.Invoice Date') }}</label>
												</div>
												<div class="col-md-8 col-sm-8">
									...
									<div class="row">
										<div class="col-md-12">
											<div class="form-group input-group">
												...
												<div class="col-md-8 col-sm-8">
													<input type="number" min="0"  class="form-control" name="po_no"  value="@if(isset($Tbl_invoice)){{ $Tbl_invoice->po_no }}@endif">
												</div>
											</div>
										</div>
									</div>
                        ...
                        ?>
                            <tr id="check_remove_product1">
                                ...
                                <td style="display:<?php if(isset($Tbl_invoice)){ if($Tbl_invoice->place_supply == $client_sel_data->state){echo '';}else{echo 'none'; } } ?>">
                                  <input type="hidden" min="0" class="form-control" onkeyup="chnage_in_product_data_update('{{ $i }}')" id="cgst{{ $i }}" name="cgst{{ $i }}" value="{{ $key->cgst }}"><div id="dis_cgst{{ $i }}">{{ $key->cgst }}%</div><div><small id="read_cgstshow_up{{ $i }}" style="color: red;"><?php echo (($key['cgst'] * $key['price'])/100)*$key['qty']; ?></small><div></div>
                                </td>

                                <td style="display:<?php if(isset($Tbl_invoice)){ if($Tbl_invoice->place_supply == $client_sel_data->state){echo '';}else{echo 'none'; } } ?>"><input type="hidden" min="0" onkeyup="chnage_in_product_data_update('{{ $i }}')" class="form-control" id="sgst{{ $i }}" name="sgst{{ $i }}" value="{{ $key->sgst }}"><div id="dis_sgst{{ $i }}">{{ $key->sgst }}%</div><div><small id="read_sgstshow_up{{ $i }}" style="color: red;"><?php echo (($key['sgst'] * $key['price'])/100)*$key['qty']; ?></small><div></div>
                                </td>


                                <td style="display:<?php if(isset($Tbl_invoice)){ if($Tbl_invoice->place_supply == $client_sel_data->state){echo 'none';}else{echo ''; } } ?>">
                                  <input type="hidden" min="0" class="form-control" onkeyup="chnage_in_product_data_update('{{ $i }}')" id="igst{{ $i }}" name="igst{{ $i }}" value="{{ $key->igst }}"><div id="dis_igst{{ $i }}">{{ $key->igst }}%</div><div><small id="read_igstshow_up{{ $i }}" style="color: red;"><?php echo (($key['igst'] * $key['price'])/100)*$key['qty']; ?></small><div></div>
                                </td>
                                <td>
                          ...
                          <div class="btn-sm btn-primary mt-1" data-toggle="modal" data-target="#Add_product_modal" style="text-align: center;">{{ trans('app.Add Product') }}</div>
												...
												<td id="tdhide_cgst" style="display:<?php if(isset($Tbl_invoice)){ if($Tbl_invoice->place_supply == $client_sel_data->state){echo '';}else{echo 'none'; } } ?>"><input type="number" min="0" id="read_cgst" class="form-control" onkeyup="chnage_in_product_data();" value="0">
                        <small id="read_cgstshow" style="color: red;"></small>
                        <input type="hidden" value="0" id="read_cgstshow_val"></td>

												<td id="tdhide_sgst" style="display:<?php if(isset($Tbl_invoice)){ if($Tbl_invoice->place_supply == $client_sel_data->state){echo '';}else{echo 'none'; } } ?>"><input type="number" min="0" id="read_sgst" class="form-control" onkeyup="chnage_in_product_data();" value="0">
                        <small id="read_sgstshow" style="color: red;"></small>
                        <input type="hidden" value="0" id="read_sgstshow_val"></td>

												<td id="tdhide_igst" style="display:<?php if(isset($Tbl_invoice)){ if($Tbl_invoice->place_supply == $client_sel_data->state){echo 'none';}else{echo ''; } } ?>"><input type="number" min="0" id="read_igst" class="form-control" onkeyup="chnage_in_product_data();" value="0">
                        <small id="read_igstshow" style="color: red;"></small>
                        <input type="hidden" value="0" id="read_igstshow_val"></td>
...
function chnage_in_product_data()
{
	...
	else
	{
      ...
      $("#read_sgstshow").html(parseFloat(stax * qty).toFixed(2));
      $("#read_cgstshow").html(parseFloat(ctax * qty).toFixed(2));
      $("#read_igstshow").html(parseFloat(itax * qty).toFixed(2));
      $("#read_sgstshow_val").val(parseFloat(stax * qty).toFixed(2));
      $("#read_cgstshow_val").val(parseFloat(ctax * qty).toFixed(2));
      $("#read_igstshow_val").val(parseFloat(itax * qty).toFixed(2));
...
function add_new_product()
{
  ...
  var read_sgstshow_val = $("#read_sgstshow_val").val();
  var read_cgstshow_val = $("#read_cgstshow_val").val();
  var read_igstshow_val = $("#read_igstshow_val").val();
	if(id == '')
	{
		swal("Please select Product first.", "", "error");
		$('#read_product_id').focus();
	...
	else
	{
		if(hsn != '' && price!= '' && qty!= '' && discount!= '' && sgst != '' && cgst != '' && igst != '')
		{
            ...
            $("#read_cgstshow").html('');
            $("#read_igstshow").html('');
            $("#read_sgstshow_val").val('0');
            $("#read_cgstshow_val").val('0');
            $("#read_igstshow_val").val('0');
...
function final_result()
{
	...
	$("#total_total").val(parseFloat(total_total-totalcgst-totalsgst-totaligst).toFixed(2));
	$("#read_total_total").html(parseFloat(total_total-totalcgst-totalsgst-totaligst).toFixed(2));
...
function chnage_in_product_data_update(no)
{
	...
	if(hsn != '' && price!= '' && qty!= '' && discount!= '')
	{
		...
		var itax = ((igst * price)/100);
    console.log(itax);
    $("#read_sgstshow_up"+no).html(parseFloat(stax * qty).toFixed(2));
    $("#read_cgstshow_up"+no).html(parseFloat(ctax * qty).toFixed(2));
    $("#read_igstshow_up"+no).html(parseFloat(itax * qty).toFixed(2));
		var total_tax = +stax + +ctax + +itax + +price;
		var amount = (total_tax * qty);
    var discount = $("#discount"+no).val();
    var discount_amount = ((discount * amount)/100);
