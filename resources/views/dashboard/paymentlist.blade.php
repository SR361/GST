...
<div class="content-wrapper">
	...
	<div class="content-header">
		<div class="container-fluid">
			<div class="row mb-2">
				<div class="col-sm-6">
					<h1 class="m-0 text-dark">{{ trans('app.Payment List') }}</h1>
				</div>
				<!-- /.col -->
				<div class="col-sm-6">
					<ol class="breadcrumb float-sm-right">
						<li class="breadcrumb-item"><a href="{{ url(app()->getLocale().'/dashboard') }}">{{ trans('app.Home') }}</a></li>
						<li class="breadcrumb-item active">{{ trans('app.Payment List') }}</li>
					</ol>
				</div>
				<!-- /.col -->
			</div>
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
       <h4 class="modal-title">{{ trans('app.Payment Info') }}</h4>
       <button type="button" class="close" data-dismiss="modal">&times;</button>

     </div>
     <div class="modal-body" id="paymentinfodata">
    </div>
  ...
  <div class="row">
   <div class="col-12">
     ...
     <form action="{{ url(app()->getLocale().'/paymentsmuldel') }}" method="post" onsubmit="return giveNotation()">
    {{ csrf_field() }}
      <div class="card">
        <div class="card-header" style="display: <?php if($delete_row == '0' && $add_row == '0'){ echo "none"; } ?>">
          ...
          <?php } ?>
          <?php if($add_row == '1'){ ?>
           <button type="button" class="btn btn-success btn-sm pull-right" data-toggle="dropdown" aria-expanded="false">
              {{ trans('app.Add Payment') }}
            </button>
            <div class="dropdown-menu" role="menu" x-placement="bottom-start" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(67px, 38px, 0px);">
              <a class="dropdown-item" href="{{ route('paymentcreate',[app()->getLocale(),'received']) }}">{{ trans('app.Received') }}</a>
              <a class="dropdown-item" href="{{ route('paymentcreate',[app()->getLocale(),'made']) }}">{{ trans('app.Made') }}</a>
            </div>
          <?php } ?>
        </div>
        
        <div class="card-body mailbox-messages " > <!-- style="overflow:scroll" -->
          <table id="payment_table" class="{{ Config::get('constants.TABLE_CLASS') }}" style="width:100%; text-align: center;">  <!-- class="table table-bordered table-hover table-striped mailbox-messages" -->
...
<script>
	var table = $('#payment_table').DataTable( {
		...
		"order": [0],
		"ajax": {
			"url":"<?= route('paymentdataProcessing',app()->getLocale()) ?>",
			"dataType":"json",
			"type":"POST",
  ...
  $('#payment_table').on( 'column-visibility.dt', function ( e, settings, column, state ) {
   ...
   else
   {
     $.ajax({  
      ...
      success:function(data)  
      {
        lswal("", "Column hide done!", "success");
        table.ajax.reload();
      }
    });
   }
	...
	function dlt_data(id)
	{
		if (confirm("Are You Sure you want to delete?")) {
			$.ajax({  
				url: "{{ url(app()->getLocale().'/paymentsdelete')}}/"+id,
				type: 'DELETE',
  ...
  {
    $.ajax({  
        url: "<?= route('paymentdatainfo',app()->getLocale()) ?>",
        type: 'POST',
        data: {id:id,"_token":"<?= csrf_token() ?>"},
        success:function(data)  
        {
         $("#paymentinfodata").html(data);
