...
<div class="content-wrapper">
	...
	<div class="content-header">
		<div class="container-fluid">
			<div class="row mb-2">
				<div class="col-sm-6">
					<h1 class="m-0 text-dark">{{ trans('app.Invoice List') }}</h1>
				</div>
				<!-- /.col -->
				<div class="col-sm-6">
					<ol class="breadcrumb float-sm-right">
						<li class="breadcrumb-item"><a href="{{ url(app()->getLocale().'/dashboard') }}">{{ trans('app.Home') }}</a></li>
						<li class="breadcrumb-item active">{{ trans('app.Invoice List') }}</li>
					</ol>
				</div>
				<!-- /.col -->
			</div>
  ...
  <div id="invoiceinfo" class="modal fade docs-example-modal-lg" role="dialog">
  <div class="modal-dialog modal-lg" style="max-width: 91%" >
    <div class="modal-content ">
      <div class="modal-header">
       <h4 class="modal-title">{{ trans('app.Invoice Info') }}</h4>
       <button type="button" class="close" data-dismiss="modal">&times;</button>

     </div>
     <div class="modal-body" id="invoiceinfodata">
    </div>
  ...
  <div class="row">
   <div class="col-12">
     ...
     <form action="{{ url(app()->getLocale().'/invoicesmuldel') }}" method="post" onsubmit="return giveNotation()">
    {{ csrf_field() }}
      <div class="card">
        <div class="card-header" style="display: <?php if($delete_row == '0' && $add_row == '0'){ echo "none"; } ?>">
          ...
          <?php } ?>
          <?php if($add_row == '1'){ ?>
           <a href="{{ route('invoices.create',app()->getLocale()) }}" class="btn btn-success btn-sm pull-right">{{ trans('app.Add Invoice') }}</a>
          <?php } ?>
        </div>
        
        <div class="card-body mailbox-messages " > <!-- style="overflow:scroll" -->
          <table id="invoice_table" class="{{ Config::get('constants.TABLE_CLASS') }}"  style="width:100%; text-align: center;"> <!-- class="table table-bordered table-hover table-striped mailbox-messages" -->
...
<script>
	var table = $('#invoice_table').DataTable( {
		...
		"order": [0],
		"ajax": {
			"url":"<?= route('invoicedataProcessing',app()->getLocale()) ?>",
			"dataType":"json",
			"type":"POST",
  ...
  {
    $.ajax({  
        url: "<?= route('invoicedatainfo',app()->getLocale()) ?>",
        type: 'POST',
        data: {id:id,"_token":"<?= csrf_token() ?>"},
        success:function(data)  
        {
