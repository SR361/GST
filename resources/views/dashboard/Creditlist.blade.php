...
<div class="content-wrapper">
	...
	<div class="content-header">
		<div class="container-fluid">
			<div class="row mb-2">
				<div class="col-sm-6">
					<h1 class="m-0 text-dark">{{ trans('app.Credit List') }}</h1>
				</div>
				<!-- /.col -->
				<div class="col-sm-6">
					<ol class="breadcrumb float-sm-right">
						<li class="breadcrumb-item"><a href="{{ url(app()->getLocale().'/dashboard') }}">{{ trans('app.Home') }}</a></li>
						<li class="breadcrumb-item active">{{ trans('app.Credit List') }}</li>
					</ol>
				</div>
				<!-- /.col -->
			</div>
  ...
  <div id="creditinfo" class="modal fade docs-example-modal-lg" role="dialog">
  <div class="modal-dialog modal-lg">
    <div class="modal-content ">
      <div class="modal-header">
       <h4 class="modal-title">{{ trans('app.Credit Info') }}</h4>
       <button type="button" class="close" data-dismiss="modal">&times;</button>

     </div>
     <div class="modal-body" id="creditinfodata">
    </div>
  ...
  <div class="row">
   <div class="col-12">
     ...
     <form action="{{ url(app()->getLocale().'/creditsmuldel') }}" method="post" onsubmit="return giveNotation()">
    {{ csrf_field() }}
      <div class="card">
        <div class="card-header" style="display: <?php if($delete_row == '0' && $add_row == '0'){ echo "none"; } ?>">
          ...
          <?php } ?>
          <?php if($add_row == '1'){ ?>
           <a href="{{ route('credits.create',app()->getLocale()) }}" class="btn btn-success btn-sm pull-right">{{ trans('app.Add') }}</a>
          <?php } ?>
        </div>
        
        <div class="card-body mailbox-messages " > <!-- style="overflow:scroll" -->
          <table id="credit_table" class="{{ Config::get('constants.TABLE_CLASS') }}"  style="width:100%; text-align: center;"> <!-- class="table table-bordered table-hover table-striped mailbox-messages" -->
...
<script>
	var table = $('#credit_table').DataTable( {
		...
		"order": [0],
		"ajax": {
			"url":"<?= route('creditdataProcessing',app()->getLocale()) ?>",
			"dataType":"json",
			"type":"POST",
    ...
    buttons: [
        ...
        },
        {
          ...
          columns: <?php if($delete_row == '0' && $update_row == '0'){ echo '[1,2,3,4,5,6]'; }else{ echo '[1,2,3,4,5,6,7]'; } ?>,
    ...
    "columnDefs": [
        ...
        ,{
          "targets": [ 7 ],
          "visible": '<?php if(in_array('7', $array_check) || ($delete_row == '0' && $update_row == '0')){ echo false; }else{ echo true; } ?>',
          "searchable": '<?php if(in_array('7', $array_check) || ($delete_row == '0' && $update_row == '0')){ echo false; }else{ echo true; } ?>'
        }]
    } );

  $('#credit_table').on( 'column-visibility.dt', function ( e, settings, column, state ) {
   var name = "Creditlist";
   ...
   else
   {
	...
	function dlt_data(id)
	{
		if (confirm("Are You Sure you want to delete?")) {
			$.ajax({  
				url: "{{ url(app()->getLocale().'/creditsdelete')}}/"+id,
				type: 'DELETE',
  ...
  {
    $.ajax({  
        url: "<?= route('creditdatainfo',app()->getLocale()) ?>",
        type: 'POST',
        data: {id:id,"_token":"<?= csrf_token() ?>"},
        success:function(data)  
        {
         $("#creditinfodata").html(data);
